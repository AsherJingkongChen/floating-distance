...
fn inner_product_exact_exact() {
  ...
  let expectation: f32 =
    ...
    12.0 + 1.0 + 12.0 + 1.0 +
    1.0 + 4.0 + 1.0 + 4.0_f32;
  let result: f32 = v0.distance(v1, Metric::InnerProduct);
  assert_eq!(result, expectation);
}

#[test]
fn inner_product_exact_noexact() {
  const LEN: (usize, usize) = (
    2 * MAX_LANE_COUNT_PER_INSTR,
...
#[test]
fn inner_product_noexact_noexact_not_equal() {
  const LEN: (usize, usize) = (
...
#[test]
fn inner_product_noexact_noexact_is_equal() {
  const LEN: (usize, usize) = (
    2 * MAX_LANE_COUNT_PER_INSTR - 3,
    2 * MAX_LANE_COUNT_PER_INSTR - 3,
  );
